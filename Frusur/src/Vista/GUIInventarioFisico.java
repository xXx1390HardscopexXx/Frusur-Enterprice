...
public class GUIInventarioFisico extends JDialog {
    ...
    private void verStock() {
        ...
        for (TipoBerrie t : TipoBerrie.values()) sb.append(" - ").append(t).append(": ").append(inv().getStock(t, ClasificacionProducto.IQF)).append("\n");
        sb.append("\nSTOCK SUBPRODUCTO (kg):\n");
        for (TipoBerrie t : TipoBerrie.values()) sb.append(" - ").append(t).append(": ").append(inv().getStock(t, ClasificacionProducto.SUBPRODUCTO)).append("\n");
    ...
    private void verPalets() {
        StringBuilder sb = new StringBuilder();
        sb.append("PALETS:\n");
        for (Palet p : inv().getPalets()) sb.append(" - ").append(p).append("\n");
        txtSalida.setText(sb.toString());
    }
